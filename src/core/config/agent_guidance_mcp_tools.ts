...
const DESCRIPTIONS: Readonly<Record<string, string>> = Object.freeze({
  ...
  vibecode_claim_intents_list: 'List the agent\'s work intents with claim detail (active/released counts, paths). Read-only.',
  vibecode_claim_intent_release: 'Release all active claims belonging to a work intent. Same-agent only. Blocked when claimed files are dirty — commit or revert first. Supports dry_run.',
