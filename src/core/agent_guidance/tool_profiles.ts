...
const PROFILES: Readonly<Record<ToolProfileId, ToolProfile>> = Object.freeze({
  ...
  team_handoff: {
    ...
    mcp_tools: [
      ...
      { name: 'vibecode_handoff_guide', reason: 'CONSUMER: a DIFFERENT next agent runs handoff guide BEFORE continuing — one onboarding_state, blocked paths still claimed by the previous agent, and per-agent safe commands. If the previous agent is not ready, do not proceed on its paths.' },
      ...
      { name: 'vibecode_claim_release', reason: 'Release YOUR OWN clean active claims when there is no active intent; use claim-release guidance, not intent-release.' },
    ],
    cli_commands: [
      ...
      { command: 'vibecode claims list --agent <agent_id> --json', reason: 'If clean active claims exist without an active intent: inspect your own claims before releasing them.' },
      { command: 'vibecode claims release --claim <claim_id> --json', reason: 'Release your own clean active claim when there is no intent to release; never release another agent’s claim.' },
    ...
    next_steps: [
      'Current agent: commit or revert dirty claimed files (commit guard dry-run first), then release your own clean intents or clean active claims, then re-run handoff prepare until handoff_ready.',
      'Next agent: handoff guide is for a different next agent before continuing — if it reports the previous agent is not ready, do not proceed on its paths; wait for the prerequisites instead.',
      ...
      'Same-agent continuation is not a cross-agent handoff — use session_recovery instead of handoff guide onboarding.',
    ...
    warnings: [
      ...
      'Never release another agent’s intent — intent release is same-agent only.',
      'Claim-only previous ownership must be cleared through the safe claims release path by the owning agent before another agent edits.',
