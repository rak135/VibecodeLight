...
function makeRepoWithRun(opts: {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      codegraph_repo_atlas_generated: Boolean(opts.withRepoAtlas),
      codegraph_repo_atlas_reason: opts.withRepoAtlas ? 'generated' : opts.reason,
      ...(opts.withRepoAtlas ? { codegraph_repo_atlas_artifact: 'scan/codegraph_repo_atlas.md' } : {}),
      ...(opts.withRepoAtlasJson ? { codegraph_repo_atlas_json_artifact: 'scan/codegraph_repo_atlas.json' } : {}),
  ...
  if (opts.withRepoAtlas) {
    fs.writeFileSync(path.join(scanDir, 'codegraph_repo_atlas.md'), '# CodeGraph-derived Repo Atlas\n', 'utf8');
    fs.writeFileSync(path.join(scanDir, 'repo_atlas.md'), '# Repo atlas\n', 'utf8');
  }
  if (opts.withRepoAtlasJson) {
    fs.writeFileSync(path.join(scanDir, 'codegraph_repo_atlas.json'), '{"ok":true}\n', 'utf8');
    fs.writeFileSync(path.join(scanDir, 'repo_atlas.json'), '{"ok":true}\n', 'utf8');
  }
...
describe('runs show CodeGraph visibility', () => {
  test('human output shows used=true CodeGraph summary and artifacts', () => {
    ...
    try {
      ...
      expect(result.stdout).toContain('CodeGraph-derived Repo Atlas: generated');
      expect(result.stdout).toContain('scan/codegraph_usage.json');
      expect(result.stdout).toContain('scan/codegraph_context.md');
      expect(result.stdout).toContain('scan/codegraph_repo_atlas.md');
      expect(result.stdout).toContain('scan/codegraph_repo_atlas.json');
  ...
  test('artifact read supports CodeGraph alias and explicit scan artifacts', () => {
    ...
    try {
      ...
      const atlas = runCli(['runs', 'show', 'latest', '--repo', fixture.repo, '--artifact', 'scan/codegraph_repo_atlas.md']);
      expect(atlas.status).toBe(0);
      expect(atlas.stdout).toContain('# CodeGraph-derived Repo Atlas');

      const legacyAtlas = runCli(['runs', 'show', 'latest', '--repo', fixture.repo, '--artifact', 'scan/repo_atlas.md']);
      expect(legacyAtlas.status).toBe(0);
      expect(legacyAtlas.stdout).toContain('# Repo atlas');
    } finally {
      fs.rmSync(fixture.repo, { recursive: true, force: true });
    }
  });
