...
- MCP exposes Agent Guidance through VibecodeMCP. The effective guidance from `%LOCALAPPDATA%/vibecodelight/agent-guidance-config.yaml` is applied to MCP server instructions and bounded per-tool description suffixes (with source, config path, and `guidance_hash` reported in the server instructions). There is no separate public guidance tool in the v1 contract; agents start with `vibecode_session_start` and `vibecode_workspace_snapshot`.
