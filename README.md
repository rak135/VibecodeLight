...
The CLI setting is stored in the global user config (`%LOCALAPPDATA%/vibecodelight/config.yaml`) as `defaults.codegraph.transport`; unset or invalid values default to `cli`. Desktop uses the same semantics from the **CodeGraph Transport: CLI / MCP / Auto** dropdown, persisted in renderer `localStorage` under `vibecode.codegraphTransport`. The stores are separate today (desktop localStorage vs CLI global config), but both represent the same conceptual setting and use the same `cli` / `mcp` / `auto` values. prompt-level transport flags are intentionally not the primary UX; CLI `prompt` and `context-build` consume the persisted setting when CodeGraph mode is `use-existing`.
